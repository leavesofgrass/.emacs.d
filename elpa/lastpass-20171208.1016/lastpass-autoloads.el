...
Prompts user for password if not logged in." t nil)

(autoload 'lastpass-status "lastpass" "\
Check LastPass status, if user is logged in or not." t nil)

(autoload 'lastpass-logout "lastpass" "\
Log out lpass.  Does not ask for confirmation." t nil)
...
Enable lastpass auth-source by adding it to `auth-sources'." t nil)
